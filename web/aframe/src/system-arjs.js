AFRAME.registerSystem('arjs', {
  ...
  init: function() {
    ...
    this.el.sceneEl.addEventListener('renderstart', function() {
      AR_EXPERIMENT.dataPoints.rendererIsReady = Date.now()
      ...
      if (
        arProfile.contextParameters.trackingBackend === 'tango' &&
        _this.data.tangoPointCloudEnabled
      ) {
        // init tangoPointCloud
        var tangoPointCloud = (_this._tangoPointCloud = new ARjs.TangoPointCloud(
          arSession
        ))
      ...
      if (arProfile.contextParameters.trackingBackend === 'tango') {
        // init tangoVideoMesh
        var tangoVideoMesh = (_this._tangoVideoMesh = new ARjs.TangoVideoMesh(
          arSession
        ))

        // override renderer.render to render tangoVideoMesh
        var rendererRenderFct = renderer.render
        renderer.render = function customRender(
          scene,
          camera,
          renderTarget,
          forceClear
          ...
          if (arProfile.contextParameters.trackingBackend === 'tango') {
            ...
            // render sceneOrtho
            rendererRenderFct.call(
              renderer,
              tangoVideoMesh._sceneOrtho,
              tangoVideoMesh._cameraOrtho,
              renderTarget,
              forceClear
            )
